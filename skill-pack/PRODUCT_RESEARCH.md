...
- **联网搜索结果为空或不相关**：不要编造产品功能。在报告中标注「未找到足够的公开信息」，输出基于 JD 内容的推断，明确标注为「推测」而非事实
- **JD 未明确提及产品名称**：根据公司名搜索主要产品线，选择与岗位职责最相关的产品作为研究对象。如无法确定，选择最可能的核心产品并在报告开头说明假设
- **搜索超时或被限流**：等待 30 秒后重试一次；仍失败则告知用户手动搜索，生成仅基于 JD 的框架报告（只含结构和提示，不含搜索结果内容）
